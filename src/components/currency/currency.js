...
import { Get_Currencies } from "../../GraphQL/GraphQL";
class Currency extends React.Component {
  ...
  componentDidUpdate() {
    ...
    if (
      localStorage.getItem("currencyIndex") != null && // to see if i decided the currency before then closed the tap
    ...
    // in the first time ever. we set state to (USD)
      ...
      localStorage.getItem("currencyIndex") === null && //to check if i opened the app ever
      ...
      return (
        <div className="dropdown" ref={this.wrapperRef}>
          <button
            ...
            className="dropbtn"
          >
            {this.props.data.currencies[this.props.currencyIndexArray].symbol}{" "}
            <img
              src={arrow}
              alt=""
              style={this.props.iscartopened ? { transform: " rotate(0deg)" } : { transform: this.state.opened ? "rotate(180deg)" : "rotate(0deg)" }}
            />
          </button>
  ...
  render() {
    return <div>{this.getCurrencies()}</div>;
  }
}

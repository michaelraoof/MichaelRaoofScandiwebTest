...
export const cartSlice = createSlice({
  ...
  reducers: {
    ...
    selectAttribute: (state, action) => {
      state.products[action.payload.indexproduct].attributes[action.payload.indexattribute].indexSelectedAttribute =
        action.payload.valueindexSelectedAttribute;
    },
    ...
    addProductToCart: (state, action) => {
      ...
      for (var productindex = 0; productindex < state.products.length; productindex++) {
        ...
        if (state.products[productindex].id === action.payload.id) {
          if (state.products[productindex].attributes.length === 0) {
            index = productindex;
          }
          for (var attindex = 0; attindex < state.products[productindex].attributes.length; attindex++) {
            if (
              state.products[productindex].attributes[attindex].indexSelectedAttribute !== action.payload.attributes[attindex].indexSelectedAttribute
            ) {
      ...
      if (index === -1) {
        //if index = -1 it means that it is the first time for the product to be added
        ...
        // if index = any num that means produc added before so we just need to ++ our state.products[index].count
